...
#ifdef __LIBMDLX_MOD__
#include "version.h"
#define DRV_MODULE_NAME "libmdlx"
#define DRV_MODULE_DESC "Medium MDLX Base Driver"
...
/*
 * mdlx device management
 * maintains a list of the mdlx devices
 */
static LIST_HEAD(mdev_list);
static DEFINE_MUTEX(mdev_mutex);

static LIST_HEAD(mdev_rcu_list);
static DEFINE_SPINLOCK(mdev_rcu_lock);

#ifndef list_last_entry
#define list_last_entry(ptr, type, member) list_entry((ptr)->prev, type, member)
#endif

static inline void mdev_list_add(struct mdlx_dev *mdev)
{
	mutex_lock(&mdev_mutex);
	if (list_empty(&mdev_list))
		mdev->idx = 0;
	else {
		struct mdlx_dev *last;

		last = list_last_entry(&mdev_list, struct mdlx_dev, list_head);
		mdev->idx = last->idx + 1;
	}
	list_add_tail(&mdev->list_head, &mdev_list);
	mutex_unlock(&mdev_mutex);

	dbg_init("dev %s, mdev 0x%p, mdlx idx %d.\n",
		 dev_name(&mdev->pdev->dev), mdev, mdev->idx);

	spin_lock(&mdev_rcu_lock);
	list_add_tail_rcu(&mdev->rcu_node, &mdev_rcu_list);
	spin_unlock(&mdev_rcu_lock);
}

#undef list_last_entry

static inline void mdev_list_remove(struct mdlx_dev *mdev)
{
	mutex_lock(&mdev_mutex);
	list_del(&mdev->list_head);
	mutex_unlock(&mdev_mutex);

	spin_lock(&mdev_rcu_lock);
	list_del_rcu(&mdev->rcu_node);
	spin_unlock(&mdev_rcu_lock);
	synchronize_rcu();
}
...
{
	struct mdlx_dev *mdev, *tmp;

	mutex_lock(&mdev_mutex);
	list_for_each_entry_safe(mdev, tmp, &mdev_list, list_head) {
		if (mdev->pdev == pdev) {
			mutex_unlock(&mdev_mutex);
			return mdev;
		}
...
EXPORT_SYMBOL_GPL(mdev_find_by_pdev);
...
static void check_nonzero_interrupt_status(struct mdlx_dev *mdev)
{
	struct interrupt_regs *reg =
		(struct interrupt_regs *)(mdev->bar[mdev->config_bar_idx] +
					  MDLX_OFS_INT_CTRL);
	u32 w;

	w = read_register(&reg->user_int_enable);
	if (w)
		pr_info("%s mdlx%d user_int_enable = 0x%08x\n",
			dev_name(&mdev->pdev->dev), mdev->idx, w);

	w = read_register(&reg->channel_int_enable);
	if (w)
		pr_info("%s mdlx%d channel_int_enable = 0x%08x\n",
			dev_name(&mdev->pdev->dev), mdev->idx, w);

	w = read_register(&reg->user_int_request);
	if (w)
		pr_info("%s mdlx%d user_int_request = 0x%08x\n",
			dev_name(&mdev->pdev->dev), mdev->idx, w);
	w = read_register(&reg->channel_int_request);
	if (w)
		pr_info("%s mdlx%d channel_int_request = 0x%08x\n",
			dev_name(&mdev->pdev->dev), mdev->idx, w);

	w = read_register(&reg->user_int_pending);
	if (w)
		pr_info("%s mdlx%d user_int_pending = 0x%08x\n",
			dev_name(&mdev->pdev->dev), mdev->idx, w);
	w = read_register(&reg->channel_int_pending);
	if (w)
		pr_info("%s mdlx%d channel_int_pending = 0x%08x\n",
			dev_name(&mdev->pdev->dev), mdev->idx, w);
}

/* channel_interrupts_enable -- Enable interrupts we are interested in */
static void channel_interrupts_enable(struct mdlx_dev *mdev, u32 mask)
{
	struct interrupt_regs *reg =
		(struct interrupt_regs *)(mdev->bar[mdev->config_bar_idx] +
					  MDLX_OFS_INT_CTRL);

	write_register(mask, &reg->channel_int_enable_w1s, MDLX_OFS_INT_CTRL);
}

/* channel_interrupts_disable -- Disable interrupts we not interested in */
static void channel_interrupts_disable(struct mdlx_dev *mdev, u32 mask)
{
	struct interrupt_regs *reg =
		(struct interrupt_regs *)(mdev->bar[mdev->config_bar_idx] +
					  MDLX_OFS_INT_CTRL);

	write_register(mask, &reg->channel_int_enable_w1c, MDLX_OFS_INT_CTRL);
}

/* user_interrupts_enable -- Enable interrupts we are interested in */
static void user_interrupts_enable(struct mdlx_dev *mdev, u32 mask)
{
	struct interrupt_regs *reg =
		(struct interrupt_regs *)(mdev->bar[mdev->config_bar_idx] +
					  MDLX_OFS_INT_CTRL);

	write_register(mask, &reg->user_int_enable_w1s, MDLX_OFS_INT_CTRL);
}

/* user_interrupts_disable -- Disable interrupts we not interested in */
static void user_interrupts_disable(struct mdlx_dev *mdev, u32 mask)
{
	struct interrupt_regs *reg =
		(struct interrupt_regs *)(mdev->bar[mdev->config_bar_idx] +
					  MDLX_OFS_INT_CTRL);

	write_register(mask, &reg->user_int_enable_w1c, MDLX_OFS_INT_CTRL);
}

/* read_interrupts -- Print the interrupt controller status */
static u32 read_interrupts(struct mdlx_dev *mdev)
{
	struct interrupt_regs *reg =
		(struct interrupt_regs *)(mdev->bar[mdev->config_bar_idx] +
					  MDLX_OFS_INT_CTRL);
...
void enable_perf(struct mdlx_engine *engine)
{
	...
	w = MDLX_PERF_AUTO | MDLX_PERF_RUN;
	write_register(w, &engine->regs->perf_ctrl,
		       (unsigned long)(&engine->regs->perf_ctrl) -
			       (unsigned long)(&engine->regs));
	read_register(&engine->regs->identifier);

	dbg_perf("IOCTL_MDLX_PERF_START\n");
}
...
static void engine_status_dump(struct mdlx_engine *engine)
{
	u32 v = engine->status;
	char buffer[256];
	char *buf = buffer;
	...
	if ((v & MDLX_STAT_DESC_STOPPED))
		len += sprintf(buf + len, "DESC_STOPPED,");
	if ((v & MDLX_STAT_DESC_COMPLETED))
		len += sprintf(buf + len, "DESC_COMPL,");

	/* common H2C & C2H */
	if ((v & MDLX_STAT_COMMON_ERR_MASK)) {
		if ((v & MDLX_STAT_ALIGN_MISMATCH))
			len += sprintf(buf + len, "ALIGN_MISMATCH ");
		if ((v & MDLX_STAT_MAGIC_STOPPED))
			len += sprintf(buf + len, "MAGIC_STOPPED ");
		if ((v & MDLX_STAT_INVALID_LEN))
			len += sprintf(buf + len, "INVLIAD_LEN ");
		if ((v & MDLX_STAT_IDLE_STOPPED))
	...
	if (engine->dir == DMA_TO_DEVICE) {
		/* H2C only */
		if ((v & MDLX_STAT_H2C_R_ERR_MASK)) {
			len += sprintf(buf + len, "R:");
			if ((v & MDLX_STAT_H2C_R_UNSUPP_REQ))
				len += sprintf(buf + len, "UNSUPP_REQ ");
			if ((v & MDLX_STAT_H2C_R_COMPL_ABORT))
				len += sprintf(buf + len, "COMPL_ABORT ");
			if ((v & MDLX_STAT_H2C_R_PARITY_ERR))
				len += sprintf(buf + len, "PARITY ");
			if ((v & MDLX_STAT_H2C_R_HEADER_EP))
				len += sprintf(buf + len, "HEADER_EP ");
			if ((v & MDLX_STAT_H2C_R_UNEXP_COMPL))
				len += sprintf(buf + len, "UNEXP_COMPL ");
			buf[len - 1] = ',';
		}

		if ((v & MDLX_STAT_H2C_W_ERR_MASK)) {
			len += sprintf(buf + len, "W:");
			if ((v & MDLX_STAT_H2C_W_DECODE_ERR))
				len += sprintf(buf + len, "DECODE_ERR ");
			if ((v & MDLX_STAT_H2C_W_SLAVE_ERR))
		...
		/* C2H only */
		if ((v & MDLX_STAT_C2H_R_ERR_MASK)) {
			len += sprintf(buf + len, "R:");
			if ((v & MDLX_STAT_C2H_R_DECODE_ERR))
				len += sprintf(buf + len, "DECODE_ERR ");
			if ((v & MDLX_STAT_C2H_R_SLAVE_ERR))
				len += sprintf(buf + len, "SLAVE_ERR ");
			buf[len - 1] = ',';
		}
	}
	...
	/* common H2C & C2H */
	if ((v & MDLX_STAT_DESC_ERR_MASK)) {
		len += sprintf(buf + len, "DESC_ERR:");
		if ((v & MDLX_STAT_DESC_UNSUPP_REQ))
			len += sprintf(buf + len, "UNSUPP_REQ ");
		if ((v & MDLX_STAT_DESC_COMPL_ABORT))
			len += sprintf(buf + len, "COMPL_ABORT ");
		if ((v & MDLX_STAT_DESC_PARITY_ERR))
			len += sprintf(buf + len, "PARITY ");
		if ((v & MDLX_STAT_DESC_HEADER_EP))
			len += sprintf(buf + len, "HEADER_EP ");
		if ((v & MDLX_STAT_DESC_UNEXP_COMPL))
			len += sprintf(buf + len, "UNEXP_COMPL ");
		buf[len - 1] = ',';
	}
 ...
 */
static int engine_status_read(struct mdlx_engine *engine, bool clear, bool dump)
{
	int rv = 0;
...
/**
 * mdlx_engine_stop() - stop an SG DMA engine
 *
 */
static int mdlx_engine_stop(struct mdlx_engine *engine)
{
	u32 w;
	...
	w = 0;
	w |= (u32)MDLX_CTRL_IE_DESC_ALIGN_MISMATCH;
	w |= (u32)MDLX_CTRL_IE_MAGIC_STOPPED;
	w |= (u32)MDLX_CTRL_IE_READ_ERROR;
	w |= (u32)MDLX_CTRL_IE_DESC_ERROR;

	if (poll_mode) {
		w |= (u32)MDLX_CTRL_POLL_MODE_WB;
	} else {
		w |= (u32)MDLX_CTRL_IE_DESC_STOPPED;
		w |= (u32)MDLX_CTRL_IE_DESC_COMPLETED;
...
static int engine_start_mode_config(struct mdlx_engine *engine)
{
	u32 w;
	...
	if (engine->mdlx_perf) {
		w = MDLX_CTRL_IE_DESC_STOPPED;
		w |= MDLX_CTRL_IE_DESC_COMPLETED;
		w |= MDLX_CTRL_IE_DESC_ALIGN_MISMATCH;
		w |= MDLX_CTRL_IE_MAGIC_STOPPED;
		w |= MDLX_CTRL_IE_IDLE_STOPPED;
		w |= MDLX_CTRL_IE_READ_ERROR;
		w |= MDLX_CTRL_IE_DESC_ERROR;
 ...
 *
 */
static struct mdlx_transfer *engine_start(struct mdlx_engine *engine)
{
	...
	// 0x4080 H2C SGDMA Descriptor Low Address	
	w = cpu_to_le32(PCI_DMA_L(transfer->desc_bus));				// byte order change
	...
	// 0x4084 H2C SGDMA Descriptor High Address			   
	w = cpu_to_le32(PCI_DMA_H(transfer->desc_bus));				// byte order change
 ...
 *
 * @engine pointer to struct mdlx_engine
 *
 */
static int engine_service_shutdown(struct mdlx_engine *engine)
{
	int rv;
	/* if the engine stopped with RUN still asserted, de-assert RUN now */
	dbg_tfr("engine just went idle, resetting RUN_STOP.\n");
	rv = mdlx_engine_stop(engine);
	if (rv < 0) {
...
static struct mdlx_transfer *engine_transfer_completion(
...
static struct mdlx_transfer *
engine_service_transfer_list(struct mdlx_engine *engine,
			     struct mdlx_transfer *transfer,
			     u32 *pdesc_completed)
{
	if (!engine) {
...
static int engine_err_handle(struct mdlx_engine *engine,
			     struct mdlx_transfer *transfer, u32 desc_completed)
{
	u32 value;
	int rv = 0;
	/*
	 ...
	 */
	if (engine->status & MDLX_STAT_BUSY) {
		value = read_register(&engine->regs->status);
		if ((value & MDLX_STAT_BUSY))
...
static struct mdlx_transfer *
engine_service_final_transfer(struct mdlx_engine *engine,
			      struct mdlx_transfer *transfer,
			      u32 *pdesc_completed)
{
	if (!engine) {
	...
	if (((engine->dir == DMA_FROM_DEVICE) &&
	     (engine->status & MDLX_STAT_C2H_ERR_MASK)) ||
	    ((engine->dir == DMA_TO_DEVICE) &&
	     (engine->status & MDLX_STAT_H2C_ERR_MASK))) {
...
static int engine_service_perf(struct mdlx_engine *engine, u32 desc_completed)
{
	if (!engine) {
		pr_err("dma engine NULL\n");
		return -EINVAL;
	...
	if (engine->mdlx_perf) {
		...
		if (engine->status & MDLX_STAT_DESC_COMPLETED) {
			engine->mdlx_perf->iterations = desc_completed;
			dbg_perf("transfer->mdlx_perf->iterations=%d\n",
				 engine->mdlx_perf->iterations);
		}

		/* a descriptor stopped the engine? */
		if (engine->status & MDLX_STAT_DESC_STOPPED) {
			engine->mdlx_perf->stopped = 1;
			/*
			 * wake any MDLX_PERF_IOCTL_STOP waiting for
			 * the performance run to finish
			 */
			xlx_wake_up(&engine->mdlx_perf_wq);
			dbg_perf("transfer->mdlx_perf stopped\n");
		}
	}
	return 0;
}

static int engine_transfer_dequeue(struct mdlx_engine *engine)
{
	struct mdlx_transfer *transfer;
...
static int engine_ring_process(struct mdlx_engine *engine)
{
	struct mdlx_result *result;
	int start;
	int eop_count = 0;
...
static int engine_service_cyclic_polled(struct mdlx_engine *engine)
{
	int eop_count;
	int rc = 0;
	struct mdlx_poll_wb *writeback_data;
	u32 sched_limit = 0;
	...
	writeback_data = (struct mdlx_poll_wb *)engine->poll_mode_addr_virt;
	...
	if (eop_count == 0) {
		...
		if ((engine->running) && !(engine->status & MDLX_STAT_BUSY)) {
			/* transfers on queue? */
			if (!list_empty(&engine->transfer_list)) {
...
static int engine_service_cyclic_interrupt(struct mdlx_engine *engine)
{
	int eop_count = 0;
	struct mdlx_transfer *xfer;
	int rv;
...
/* must be called with engine->lock already acquired */
static int engine_service_cyclic(struct mdlx_engine *engine)
{
	int rc = 0;
...
static int engine_service_resume(struct mdlx_engine *engine)
{
	struct mdlx_transfer *transfer_started;
...
static int engine_service(struct mdlx_engine *engine, int desc_writeback)
{
	...
	if (poll_mode) {
		wb_data = (struct mdlx_poll_wb *)engine->poll_mode_addr_virt;
...
static void engine_service_work(struct work_struct *work)
{
	...
	engine = container_of(work, struct mdlx_engine, work);
	if (engine->magic != MAGIC_ENGINE) {
		pr_err("%s has invalid magic number %lx\n", engine->name,
		       engine->magic);
		return;
	...
	if (engine->mdev->msix_enabled) {
		...
		channel_interrupts_enable(engine->mdev, engine->irq_bitmask);
...
static u32 engine_service_wb_monitor(struct mdlx_engine *engine,
				     u32 expected_wb)
{
	struct mdlx_poll_wb *wb_data;
	u32 desc_wb = 0;
...
int engine_service_poll(struct mdlx_engine *engine,
...
static irqreturn_t user_irq_service(int irq, struct mdlx_user_irq *user_irq)
{
	unsigned long flags;
...
/*
 * mdlx_isr() - Interrupt handler
 *
 * @dev_id pointer to mdlx_dev
 */
static irqreturn_t mdlx_isr(int irq, void *dev_id)
{
	u32 ch_irq;
	u32 user_irq;
	u32 mask;
	...
	mdev = (struct mdlx_dev *)dev_id;

	if (!mdev) {
		WARN_ON(!mdev);
		dbg_irq("%s(irq=%d) mdev=%p ??\n", __func__, irq, mdev);
		return IRQ_NONE;
	}

	irq_regs = (struct interrupt_regs *)(mdev->bar[mdev->config_bar_idx] +
					     MDLX_OFS_INT_CTRL);
	 ...
	 */
	if (ch_irq)
		channel_interrupts_disable(mdev, ch_irq);
	...
	if (user_irq) {
		int user = 0;
		u32 mask = 1;
		int max = mdev->h2c_channel_max;

		for (; user < max && user_irq; user++, mask <<= 1) {
			if (user_irq & mask) {
				user_irq &= ~mask;
				user_irq_service(irq, &mdev->user_irq[user]);
			}
		}
	}

	mask = ch_irq & mdev->mask_irq_h2c;
	if (mask) {
		int channel = 0;
		int max = mdev->h2c_channel_max;
		...
		for (channel = 0; channel < max && mask; channel++) {
			struct mdlx_engine *engine = &mdev->engine_h2c[channel];
	...
	mask = ch_irq & mdev->mask_irq_c2h;
	if (mask) {
		int channel = 0;
		int max = mdev->c2h_channel_max;

		/* iterate over C2H (PCIe write) */
		for (channel = 0; channel < max && mask; channel++) {
			struct mdlx_engine *engine = &mdev->engine_c2h[channel];
...
/*
 * mdlx_user_irq() - Interrupt handler for user interrupts in MSI-X mode
 *
 * @dev_id pointer to mdlx_dev
 */
static irqreturn_t mdlx_user_irq(int irq, void *dev_id)
{
	struct mdlx_user_irq *user_irq;
	...
	user_irq = (struct mdlx_user_irq *)dev_id;

	return user_irq_service(irq, user_irq);
}

/*
 * mdlx_channel_irq() - Interrupt handler for channel interrupts in MSI-X mode
 *
 * @dev_id pointer to mdlx_dev
 */
static irqreturn_t mdlx_channel_irq(int irq, void *dev_id)
{
	struct mdlx_dev *mdev;
	struct mdlx_engine *engine;
	struct interrupt_regs *irq_regs;
	...
	engine = (struct mdlx_engine *)dev_id;
	mdev = engine->mdev;
 ...
 */
static void unmap_bars(struct mdlx_dev *mdev, struct pci_dev *dev)
{
	int i;

	for (i = 0; i < MDLX_BAR_NUM; i++) {
		/* is this BAR mapped? */
		if (mdev->bar[i]) {
			/* unmap BAR */
			pci_iounmap(dev, mdev->bar[i]);
			/* mark as unmapped */
			mdev->bar[i] = NULL;
		}
	}
...
static int map_single_bar(struct mdlx_dev *mdev, struct pci_dev *dev, int idx)
{
	...
	mdev->bar[idx] = pci_iomap(dev, idx, map_len);
	...
	pr_info("BAR%d at 0x%llx mapped at 0x%p, length=%llu(/%llu)\n", idx,
		(u64)bar_start, mdev->bar[idx], (u64)map_len, (u64)bar_len);

	return (int)map_len;
}

static int is_config_bar(struct mdlx_dev *mdev, int idx)
{
	u32 irq_id = 0;
	u32 cfg_id = 0;
	int flag = 0;
	u32 mask = 0xffff0000; /* Compare only MDLX ID's not Version number */
	struct interrupt_regs *irq_regs =
		(struct interrupt_regs *)(mdev->bar[idx] + MDLX_OFS_INT_CTRL);
	struct config_regs *cfg_regs =
		(struct config_regs *)(mdev->bar[idx] + MDLX_OFS_CONFIG);
	...
	if (((irq_id & mask) == IRQ_BLOCK_ID) &&
		...
		//dbg_init("BAR %d is the MDLX config BAR\n", idx);
    	pr_info("BAR %d is the MDLX config BAR: 0x%x, 0x%x.\n", 
    		idx, irq_id, cfg_id);
		flag = 1;
	} else {
		//dbg_init("BAR %d is NOT the MDLX config BAR: 0x%x, 0x%x.\n",
    	pr_info("BAR %d is NOT the MDLX config BAR: 0x%x, 0x%x.\n",
			idx, irq_id, cfg_id);
		flag = 0;
	}
...
#ifndef MDLX_CONFIG_BAR_NUM
static int identify_bars(struct mdlx_dev *mdev, int *bar_id_list, int num_bars,
			 int config_bar_pos)
{
	/*
	 * The following logic identifies which BARs contain what functionality
	 * based on the position of the MDLX config BAR and the number of BARs
	 * detected. The rules are that the user logic and bypass logic BARs
	 * are optional.  When both are present, the MDLX config BAR will be the
	 ...
	 * MDLX config BAR is detected first or last.  When both are omitted,
	 * only the MDLX config BAR is present.  This somewhat convoluted
	...
	dbg_init("mdev 0x%p, bars %d, config at %d.\n", mdev, num_bars,
		 config_bar_pos);
	...
	case 2:
		if (config_bar_pos == 0) {
			mdev->bypass_bar_idx = bar_id_list[1];
		} else if (config_bar_pos == 1) {
			mdev->user_bar_idx = bar_id_list[0];
		} else {
			pr_info("2, MDLX config BAR unexpected %d.\n",
				config_bar_pos);
		}
		break;
	...
	case 4:
		if ((config_bar_pos == 1) || (config_bar_pos == 2)) {
			...
			mdev->bypass_bar_idx = bar_id_list[num_bars - 1];
		} else {
			pr_info("3/4, MDLX config BAR unexpected %d.\n",
				config_bar_pos);
		}
		break;
	...
	default:
		...
		pr_info("Unexpected # BARs (%d), MDLX config BAR only.\n",
			num_bars);
		break;
	}
	pr_info("%d BARs: config %d, user %d, bypass %d.\n", num_bars,
		config_bar_pos, mdev->user_bar_idx, mdev->bypass_bar_idx);
	return 0;
}
#endif
 ...
 */
static int map_bars(struct mdlx_dev *mdev, struct pci_dev *dev)
{
	int rv;

#ifdef MDLX_CONFIG_BAR_NUM
	rv = map_single_bar(mdev, dev, MDLX_CONFIG_BAR_NUM);
	if (rv <= 0) {
		pr_info("%s, map config bar %d failed, %d.\n",
			dev_name(&dev->dev), MDLX_CONFIG_BAR_NUM, rv);
		return -EINVAL;
	}

	if (is_config_bar(mdev, MDLX_CONFIG_BAR_NUM) == 0) {
		pr_info("%s, unable to identify config bar %d.\n",
			dev_name(&dev->dev), MDLX_CONFIG_BAR_NUM);
		return -EINVAL;
	}
	mdev->config_bar_idx = MDLX_CONFIG_BAR_NUM;

	return 0;
#else
	int i;
	int bar_id_list[MDLX_BAR_NUM];
	int bar_id_idx = 0;
	...
	for (i = 0; i < MDLX_BAR_NUM; i++) {
		...
		bar_len = map_single_bar(mdev, dev, i);
		if (bar_len == 0) {
			continue;
		} else if (bar_len < 0) {
			rv = -EINVAL;
		...
		/* Try to identify BAR as MDLX control BAR */
		if ((bar_len >= MDLX_BAR_SIZE) && (mdev->config_bar_idx < 0)) {
			if (is_config_bar(mdev, i)) {
				mdev->config_bar_idx = i;
				config_bar_pos = bar_id_idx;
				pr_info("config bar %d, pos %d.\n",
					mdev->config_bar_idx, config_bar_pos);
			}
		}
    else pr_info(">> need to search config_bar\n");

		bar_id_list[bar_id_idx] = i;
		bar_id_idx++;
	}

	/* The MDLX config BAR must always be present */
	if (mdev->config_bar_idx < 0) {
		pr_info("Failed to detect MDLX config BAR\n");
		rv = -EINVAL;
		goto fail;
	}
	//else pr_info("Succeed to detect MDLX config BAR\n");

	rv = identify_bars(mdev, bar_id_list, bar_id_idx, config_bar_pos);
	if (rv < 0) {
		pr_err("Failed to identify bars\n");
		return rv;
	}
	//else pr_info("Succeed to identify bars\n");
...
static void disable_msi_msix(struct mdlx_dev *mdev, struct pci_dev *pdev)
{
	if (mdev->msix_enabled) {
		pci_disable_msix(pdev);
		mdev->msix_enabled = 0;
	} else if (mdev->msi_enabled) {
		pci_disable_msi(pdev);
		mdev->msi_enabled = 0;
	}
}

static int enable_msi_msix(struct mdlx_dev *mdev, struct pci_dev *pdev)
{
	int rv = 0;
	...
	if (!interrupt_mode && msi_msix_capable(pdev, PCI_CAP_ID_MSIX)) {
		int req_nvec = mdev->c2h_channel_max + mdev->h2c_channel_max +
			       mdev->user_max;
		...
		for (i = 0; i < req_nvec; i++)
			mdev->entry[i].entry = i;

		rv = pci_enable_msix(pdev, mdev->entry, req_nvec);
#endif
		if (rv < 0)
...
static void prog_irq_msix_user(struct mdlx_dev *mdev, bool clear)
{
	/* user */
	struct interrupt_regs *int_regs =
		(struct interrupt_regs *)(mdev->bar[mdev->config_bar_idx] +
					  MDLX_OFS_INT_CTRL);
	u32 i = mdev->c2h_channel_max + mdev->h2c_channel_max;
	u32 max = i + mdev->user_max;
	int j;
...
static void prog_irq_msix_channel(struct mdlx_dev *mdev, bool clear)
{
	struct interrupt_regs *int_regs =
		(struct interrupt_regs *)(mdev->bar[mdev->config_bar_idx] +
					  MDLX_OFS_INT_CTRL);
	u32 max = mdev->c2h_channel_max + mdev->h2c_channel_max;
	u32 i;
	int j;
...
static void irq_msix_channel_teardown(struct mdlx_dev *mdev)
{
	struct mdlx_engine *engine;
	int j = 0;
	int i = 0;
	...
	prog_irq_msix_channel(mdev, 1);

	engine = mdev->engine_h2c;
	for (i = 0; i < mdev->h2c_channel_max; i++, j++, engine++) {
	...
	engine = mdev->engine_c2h;
	for (i = 0; i < mdev->c2h_channel_max; i++, j++, engine++) {
		if (!engine->msix_irq_line)
			break;
		dbg_sg("Release IRQ#%d for engine %p\n", engine->msix_irq_line,
		       engine);
...
static int irq_msix_channel_setup(struct mdlx_dev *mdev)
{
	int i;
	int j;
	int rv = 0;
	...
	j = mdev->h2c_channel_max;
	engine = mdev->engine_h2c;
	for (i = 0; i < mdev->h2c_channel_max; i++, engine++) {
#if KERNEL_VERSION(4, 12, 0) <= LINUX_VERSION_CODE
		vector = pci_irq_vector(mdev->pdev, i);
#else
		vector = mdev->entry[i].vector;
#endif
		rv = request_irq(vector, mdlx_channel_irq, 0, mdev->mod_name,
				 engine);
		if (rv) {
	...
	for (i = 0; i < mdev->c2h_channel_max; i++, j++, engine++) {
#if KERNEL_VERSION(4, 12, 0) <= LINUX_VERSION_CODE
		vector = pci_irq_vector(mdev->pdev, j);
#else
		vector = mdev->entry[j].vector;
#endif
		rv = request_irq(vector, mdlx_channel_irq, 0, mdev->mod_name,
				 engine);
		if (rv) {
...
static void irq_msix_user_teardown(struct mdlx_dev *mdev)
{
	int i;
	int j;
	...
	j = mdev->h2c_channel_max + mdev->c2h_channel_max;

	prog_irq_msix_user(mdev, 1);

	for (i = 0; i < mdev->user_max; i++, j++) {
#if KERNEL_VERSION(4, 12, 0) <= LINUX_VERSION_CODE
		u32 vector = pci_irq_vector(mdev->pdev, j);
#else
		u32 vector = mdev->entry[j].vector;
#endif
		dbg_init("user %d, releasing IRQ#%d\n", i, vector);
		free_irq(vector, &mdev->user_irq[i]);
	}
}

static int irq_msix_user_setup(struct mdlx_dev *mdev)
{
	int i;
	int j = mdev->h2c_channel_max + mdev->c2h_channel_max;
	int rv = 0;
	...
	for (i = 0; i < mdev->user_max; i++, j++) {
		...
		u32 vector = mdev->entry[j].vector;
#endif
		rv = request_irq(vector, mdlx_user_irq, 0, mdev->mod_name,
				 &mdev->user_irq[i]);
		if (rv) {
...
static int irq_msi_setup(struct mdlx_dev *mdev, struct pci_dev *pdev)
{
	int rv;

	mdev->irq_line = (int)pdev->irq;
	rv = request_irq(pdev->irq, mdlx_isr, 0, mdev->mod_name, mdev);
	if (rv)
...
static int irq_legacy_setup(struct mdlx_dev *mdev, struct pci_dev *pdev)
{
	u32 w;
	u8 val;
	void *reg;
...
static void irq_teardown(struct mdlx_dev *mdev)
{
	if (mdev->msix_enabled) {
		irq_msix_channel_teardown(mdev);
		irq_msix_user_teardown(mdev);
	} else if (mdev->irq_line != -1) {
		dbg_init("Releasing IRQ#%d\n", mdev->irq_line);
		free_irq(mdev->irq_line, mdev);
	}
}

static int irq_setup(struct mdlx_dev *mdev, struct pci_dev *pdev)
{
	pci_keep_intx_enabled(pdev);

	if (mdev->msix_enabled) {
		int rv = irq_msix_channel_setup(mdev);

		if (rv)
			return rv;
		rv = irq_msix_user_setup(mdev);
		if (rv)
			return rv;
		prog_irq_msix_channel(mdev, 0);
		prog_irq_msix_user(mdev, 0);

		return 0;
	} else if (mdev->msi_enabled)
		return irq_msi_setup(mdev, pdev);

	return irq_legacy_setup(mdev, pdev);
}

#ifdef __LIBMDLX_DEBUG__
static void dump_desc(struct mdlx_desc *desc_virt)
{
	int j;
	u32 *p = (u32 *)desc_virt;
...
static void transfer_dump(struct mdlx_transfer *transfer)
{
	int i;
	struct mdlx_desc *desc_virt = transfer->desc_virt;
 ...
 */
static int transfer_desc_init(struct mdlx_transfer *transfer, int count)
{
	struct mdlx_desc *desc_virt = transfer->desc_virt;
	...
	if (count > MDLX_TRANSFER_MAX_DESC) {
		pr_err("Engine cannot transfer more than %d descriptors\n",
		       MDLX_TRANSFER_MAX_DESC);
		return -EINVAL;
	}
	...
	for (i = 0; i < count - 1; i++) {
		...
		desc_bus += sizeof(struct mdlx_desc);
...
/* mdlx_desc_link() - Link two descriptors
 *
 ...
 */
static void mdlx_desc_link(struct mdlx_desc *first, struct mdlx_desc *second,
...
/* mdlx_desc_adjacent -- Set how many descriptors are adjacent to this one */
static void mdlx_desc_adjacent(struct mdlx_desc *desc, int next_adjacent)
{
	int extra_adj = 0;
...
/* mdlx_desc_control -- Set complete control field of a descriptor. */
static int mdlx_desc_control_set(struct mdlx_desc *first, u32 control_field)
{
...
/* mdlx_desc_clear -- Clear bits in control field of a descriptor. */
static int mdlx_desc_control_clear(struct mdlx_desc *first, u32 clear_mask)
{
	/* remember magic and adjacent number */
...
/* mdlx_desc_done - recycle cache-coherent linked list of descriptors.
 *
 ...
 */
static inline void mdlx_desc_done(struct mdlx_desc *desc_virt, int count)
{
	memset(desc_virt, 0, count * sizeof(struct mdlx_desc));
}

/* mdlx_desc() - Fill a descriptor with the transfer details
 *
 ...
 */
static void mdlx_desc_set(struct mdlx_desc *desc, dma_addr_t rc_bus_addr,
 ...
 */
static int transfer_abort(struct mdlx_engine *engine,
			  struct mdlx_transfer *transfer)
{
	struct mdlx_transfer *head;
	...
	head = list_entry(engine->transfer_list.next, struct mdlx_transfer,
			  entry);
	if (head == transfer)
		list_del(engine->transfer_list.next);
	else
 ...
 */
static int transfer_queue(struct mdlx_engine *engine,
			  struct mdlx_transfer *transfer)
{
	int rv = 0;
	struct mdlx_transfer *transfer_started;
	...
	mdev = engine->mdev;
	if (mdlx_device_flag_check(mdev, MDEV_FLAG_OFFLINE)) {
...
static void engine_alignments(struct mdlx_engine *engine)
{
	u32 w;
	u32 align_bytes;
...
static void engine_free_resource(struct mdlx_engine *engine)
{
	struct mdlx_dev *mdev = engine->mdev;
	...
	if (engine->poll_mode_addr_virt) {
		...
		dma_free_coherent(&mdev->pdev->dev, sizeof(struct mdlx_poll_wb),
	...
	if (engine->desc) {
		dbg_init("device %s, engine %s pre-alloc desc 0x%p,0x%llx.\n",
			 dev_name(&mdev->pdev->dev), engine->name, engine->desc,
			 engine->desc_bus);
		dma_free_coherent(&mdev->pdev->dev,
				  MDLX_TRANSFER_MAX_DESC *
					  sizeof(struct mdlx_desc),
	...
	if (engine->cyclic_result) {
		dma_free_coherent(
			&mdev->pdev->dev,
			MDLX_TRANSFER_MAX_DESC * sizeof(struct mdlx_result),
			engine->cyclic_result, engine->cyclic_result_bus);
		engine->cyclic_result = NULL;
	}
}

static int engine_destroy(struct mdlx_dev *mdev, struct mdlx_engine *engine)
{
	if (!mdev) {
		pr_err("Invalid mdev\n");
		return -EINVAL;
	...
	if (enable_credit_mp && engine->streaming &&
		...
		struct sgdma_common_regs *reg =
			(struct sgdma_common_regs
				 *)(mdev->bar[mdev->config_bar_idx] +
				    (0x6 * TARGET_SPACING));
		write_register(reg_value, &reg->credit_mode_enable_w1c, 0);
	}

	if (poll_mode)
		mdlx_thread_remove_work(engine);

	/* Release memory use for descriptor writebacks */
	engine_free_resource(engine);

	memset(engine, 0, sizeof(struct mdlx_engine));
...
{
	int rv;
	struct mdlx_transfer *transfer = 0;
	int size = engine->mdlx_perf->transfer_size;
	...
	if (!list_empty(&engine->transfer_list)) {
		...
		if (transfer->cyclic) {
			...
			if (engine->perf_buf_virt)
				dma_free_coherent(&engine->mdev->pdev->dev, size,
...
static int engine_writeback_setup(struct mdlx_engine *engine)
{
	u32 w;
	struct mdlx_dev *mdev;
	struct mdlx_poll_wb *writeback;
	 ...
	 */
	writeback = (struct mdlx_poll_wb *)engine->poll_mode_addr_virt;
 ...
 *
 * @dev Pointer to pci_dev
 * @offset byte address offset in BAR[mdev->config_bar_idx] resource for the
 * SG DMA * controller registers.
 * @dir: DMA_TO/FROM_DEVICE
 * @streaming Whether the engine is attached to AXI ST (rather than MM)
 */
static int engine_init_regs(struct mdlx_engine *engine)
{
	u32 reg_value;
	int rv = 0;

	write_register(MDLX_CTRL_NON_INCR_ADDR, &engine->regs->control_w1c,
	...
	reg_value = MDLX_CTRL_IE_DESC_ALIGN_MISMATCH;
	reg_value |= MDLX_CTRL_IE_MAGIC_STOPPED;
	reg_value |= MDLX_CTRL_IE_MAGIC_STOPPED;
	reg_value |= MDLX_CTRL_IE_READ_ERROR;
	reg_value |= MDLX_CTRL_IE_DESC_ERROR;
	...
	if (poll_mode) {
		...
		reg_value |= MDLX_CTRL_IE_DESC_STOPPED;
		reg_value |= MDLX_CTRL_IE_DESC_COMPLETED;
	}
...
static int engine_alloc_resource(struct mdlx_engine *engine)
{
	struct mdlx_dev *mdev = engine->mdev;

	engine->desc = dma_alloc_coherent(&mdev->pdev->dev,
					  MDLX_TRANSFER_MAX_DESC *
						  sizeof(struct mdlx_desc),
					  &engine->desc_bus, GFP_KERNEL);
	if (!engine->desc) {
		pr_warn("dev %s, %s pre-alloc desc OOM.\n",
			dev_name(&mdev->pdev->dev), engine->name);
		goto err_out;
	}

	if (poll_mode) {
		engine->poll_mode_addr_virt =
			dma_alloc_coherent(&mdev->pdev->dev,
					   sizeof(struct mdlx_poll_wb),
...
static int engine_init(struct mdlx_engine *engine, struct mdlx_dev *mdev,
		       int offset, enum dma_data_direction dir, int channel)
{
	...
	engine->irq_bitmask = (1 << MDLX_ENG_IRQ_NUM) - 1;
	engine->irq_bitmask <<= (mdev->engines_num * MDLX_ENG_IRQ_NUM);
	engine->bypass_offset = mdev->engines_num * BYPASS_MODE_SPACING;

	/* parent */
	engine->mdev = mdev;
	/* register address */
	engine->regs = (mdev->bar[mdev->config_bar_idx] + offset);
	engine->sgdma_regs = mdev->bar[mdev->config_bar_idx] + offset +
	...
	engine->dir = dir;
	sprintf(engine->name, "%d-%s%d-%s", mdev->idx,
	...
	if (dir == DMA_TO_DEVICE)
		mdev->mask_irq_h2c |= engine->irq_bitmask;
	else
		mdev->mask_irq_c2h |= engine->irq_bitmask;
	mdev->engines_num++;
	...
	if (poll_mode)
		mdlx_thread_add_work(engine);

	return 0;
}

/* transfer_destroy() - free transfer */
static void transfer_destroy(struct mdlx_dev *mdev, struct mdlx_transfer *xfer)
{
    /* free descriptors */
	mdlx_desc_done(xfer->desc_virt, xfer->desc_num);

	if (xfer->last_in_request && (xfer->flags & XFER_FLAG_NEED_UNMAP)) {
		struct sg_table *sgt = xfer->sgt;

		if (sgt->nents) {
			pci_unmap_sg(mdev->pdev, sgt->sgl, sgt->nents,
				     xfer->dir);
			sgt->nents = 0;
		}
	}
...
static int transfer_build(struct mdlx_engine *engine,
			  struct mdlx_request_cb *req, struct mdlx_transfer *xfer, unsigned int desc_max)
{
	...
	for (; i < desc_max; i++, j++, sdesc++) {
		...
		mdlx_desc_set(xfer->desc_virt + j, sdesc->addr, req->ep_addr,
			      sdesc->len, xfer->dir);
		xfer->len += sdesc->len;
...
static int transfer_init(struct mdlx_engine *engine, struct mdlx_request_cb *req, struct mdlx_transfer *xfer)
{
	...
	xfer->desc_bus = engine->desc_bus + (sizeof(struct mdlx_desc) * engine->desc_idx);
	xfer->res_bus = engine->cyclic_result_bus + (sizeof(struct mdlx_result) * engine->desc_idx);
	xfer->desc_index = engine->desc_idx;


	/* TODO: Need to handle desc_used >= MDLX_TRANSFER_MAX_DESC for aio calls */

	if ((engine->desc_idx + desc_max) >= MDLX_TRANSFER_MAX_DESC )
		desc_max = MDLX_TRANSFER_MAX_DESC - engine->desc_idx;
	...
	control = MDLX_DESC_STOPPED;
	control |= MDLX_DESC_EOP;
	control |= MDLX_DESC_COMPLETED;
	mdlx_desc_control_set(xfer->desc_virt + last, control);

	xfer->desc_num = desc_max;
	engine->desc_idx = (engine->desc_idx + desc_max) % MDLX_TRANSFER_MAX_DESC;
	engine->desc_used += desc_max ;

	/* fill in adjacent numbers */
	for (i = 0; i < xfer->desc_num; i++)
		mdlx_desc_adjacent(xfer->desc_virt + i, xfer->desc_num - i - 1);
...
static int transfer_init_cyclic(struct mdlx_engine *engine,
			 struct mdlx_request_cb *req, struct mdlx_transfer *xfer)
{
	...
	control |= MDLX_DESC_EOP;
	control |= MDLX_DESC_COMPLETED;
	rv = mdlx_desc_control_set(xfer->desc_virt + desc_max - 1, control);
	if (rv < 0) {
...
static void mdlx_request_cb_dump(struct mdlx_request_cb *req)
{
	int i;
...
static void mdlx_request_free(struct mdlx_request_cb *req)
{
...
static struct mdlx_request_cb *mdlx_request_alloc(unsigned int sdesc_nr)
{
	struct mdlx_request_cb *req;
	unsigned int size = sizeof(struct mdlx_request_cb) +
...
static struct mdlx_request_cb *mdlx_init_request(struct sg_table *sgt,
						 u64 ep_addr)
{
	struct mdlx_request_cb *req;
	struct scatterlist *sg = sgt->sgl;
	...
	if (j > max) {
		pr_err("Cannot transfer more than supported length %d\n",
		       desc_blen_max);
		mdlx_request_free(req);
		return NULL;
	}
	req->sw_desc_cnt = j;
#ifdef __LIBMDLX_DEBUG__
	mdlx_request_cb_dump(req);
#endif
	return req;
}

ssize_t mdlx_xfer_submit(void *dev_hndl, int channel, bool write, u64 ep_addr,
			 struct sg_table *sgt, bool dma_mapped, int timeout_ms)
{
	struct mdlx_dev *mdev = (struct mdlx_dev *)dev_hndl;
	struct mdlx_engine *engine;
	...
	struct mdlx_request_cb *req = NULL;
	struct mdlx_result *result;

	if (!dev_hndl)
		return -EINVAL;

	if (debug_check_dev_hndl(__func__, mdev->pdev, dev_hndl) < 0)
		return -EINVAL;

	if (write == 1) {
		if (channel >= mdev->h2c_channel_max) {
			pr_err("H2C channel %d >= %d.\n", channel,
				mdev->h2c_channel_max);
			return -EINVAL;
		}
		engine = &mdev->engine_h2c[channel];
	} else if (write == 0) {
		if (channel >= mdev->c2h_channel_max) {
			pr_err("C2H channel %d >= %d.\n", channel,
				mdev->c2h_channel_max);
			return -EINVAL;
		}
		engine = &mdev->engine_c2h[channel];
	}
	...
	if (!dma_mapped) {
		nents = pci_map_sg(mdev->pdev, sg, sgt->orig_nents, dir);
		if (!nents) {
	...
	req = mdlx_init_request(sgt, ep_addr);
	if (!req) {
		rv = -ENOMEM;
		goto unmap_sgl;
	}
	...
	while (nents) {
		...
		rv = transfer_queue(engine, xfer);				// send data
		if (rv < 0) {
			mutex_unlock(&engine->desc_lock);
		...
		} else {							// interrupt mode, sleep process until condition(interrupt)
...
unmap_sgl:
	if (!dma_mapped && sgt->nents) {
		pci_unmap_sg(mdev->pdev, sgt->sgl, sgt->orig_nents, dir);
		sgt->nents = 0;
	}
...
EXPORT_SYMBOL_GPL(mdlx_xfer_submit);

ssize_t mdlx_xfer_completion(void *cb_hndl, void *dev_hndl, int channel, bool write, u64 ep_addr,
			struct sg_table *sgt, bool dma_mapped, int timeout_ms)
{
...
EXPORT_SYMBOL_GPL(mdlx_xfer_completion);


ssize_t mdlx_xfer_submit_nowait(void *cb_hndl, void *dev_hndl, int channel, bool write, u64 ep_addr,
			struct sg_table *sgt, bool dma_mapped, int timeout_ms)
{
	struct mdlx_dev *mdev = (struct mdlx_dev *)dev_hndl;
	struct mdlx_engine *engine;
...
EXPORT_SYMBOL_GPL(mdlx_xfer_submit_nowait);


int mdlx_performance_submit(struct mdlx_dev *mdev, struct mdlx_engine *engine)
{
	u32 max_consistent_size = MDLX_PERF_NUM_DESC * 32 * 1024; /* 1024 pages, 4MB */
	struct mdlx_transfer *transfer;
	u64 ep_addr = 0;
	int num_desc_in_a_loop = MDLX_PERF_NUM_DESC;
	int size_in_desc = engine->mdlx_perf->transfer_size;
	...
	engine->perf_buf_virt = dma_alloc_coherent(&mdev->pdev->dev, size_in_desc,
						   &engine->perf_buf_bus,
					 GFP_KERNEL);
	if (!engine->perf_buf_virt) {
	...
	transfer = kzalloc(sizeof(struct mdlx_transfer), GFP_KERNEL);
	if (!transfer) {
	...
	if (!engine->desc) {
		engine->desc = dma_alloc_coherent(
			&mdev->pdev->dev,
			num_desc_in_a_loop * sizeof(struct mdlx_desc),
			&engine->desc_bus, GFP_KERNEL);
		if (!engine->desc) {
	...
	for (i = 0; i < transfer->desc_num; i++) {
		struct mdlx_desc *desc = transfer->desc_virt + i;
		dma_addr_t rc_bus_addr = engine->perf_buf_bus/* +
				(size_in_desc * i)*/;

		/* fill in descriptor entry with transfer details */
		mdlx_desc_set(desc, rc_bus_addr, ep_addr, size_in_desc,
			      engine->dir);
	}

	/* stop engine and request interrupt on last descriptor */
	rv = mdlx_desc_control_set(transfer->desc_virt, 0);
	if (rv < 0) {
		pr_err("Failed to set desc control\n");
		goto err_dma_desc;
	}
	/* create a linked loop */
	mdlx_desc_link(transfer->desc_virt + transfer->desc_num - 1,
	...
	dbg_perf("Queueing MDLX I/O %s request for performance measurement.\n",
...
err_engine_transfer:
	if (engine->perf_buf_virt)
		dma_free_coherent(&mdev->pdev->dev, size_in_desc, engine->perf_buf_virt,
				  engine->perf_buf_bus);
	engine->perf_buf_virt = NULL;
	return rv;
}
EXPORT_SYMBOL_GPL(mdlx_performance_submit);

static struct mdlx_dev *alloc_dev_instance(struct pci_dev *pdev)
{
	int i;
	struct mdlx_dev *mdev;
	struct mdlx_engine *engine;
	...
	mdev = kzalloc(sizeof(struct mdlx_dev), GFP_KERNEL);
	if (!mdev) {
		pr_info("OOM, mdlx_dev.\n");
		return NULL;
	}
	spin_lock_init(&mdev->lock);

	mdev->magic = MAGIC_DEVICE;
	mdev->config_bar_idx = -1;
	mdev->user_bar_idx = -1;
	mdev->bypass_bar_idx = -1;
	mdev->irq_line = -1;
	...
	for (i = 0; i < 16; i++) {
		mdev->user_irq[i].mdev = mdev;
		spin_lock_init(&mdev->user_irq[i].events_lock);
		init_waitqueue_head(&mdev->user_irq[i].events_wq);
		mdev->user_irq[i].handler = NULL;
		mdev->user_irq[i].user_idx = i; /* 0 based */
	}

	engine = mdev->engine_h2c;
	for (i = 0; i < MDLX_CHANNEL_NUM_MAX; i++, engine++) {
...
static int request_regions(struct mdlx_dev *mdev, struct pci_dev *pdev)
{
	int rv;
	...
	rv = pci_request_regions(pdev, mdev->mod_name);
	/* could not request all regions? */
	if (rv) {
		dbg_init("pci_request_regions() = %d, device in use?\n", rv);
		/* assume device is in use so do not disable it later */
		mdev->regions_in_use = 1;
	} else {
		mdev->got_regions = 1;
	}
...
static int set_dma_mask(struct pci_dev *pdev)
{
	...
	/* 64-bit addressing capability for MDLX? */
...
static void remove_engines(struct mdlx_dev *mdev)
{
	struct mdlx_engine *engine;
	int i;
	int rv;
	...
	for (i = 0; i < mdev->h2c_channel_max; i++) {
		engine = &mdev->engine_h2c[i];
		if (engine->magic == MAGIC_ENGINE) {
			dbg_sg("Remove %s, %d", engine->name, i);
			rv = engine_destroy(mdev, engine);
			if (rv < 0)
	...
	for (i = 0; i < mdev->c2h_channel_max; i++) {
...
static int probe_for_engine(struct mdlx_dev *mdev, enum dma_data_direction dir,
			    int channel)
{
	 ...
	 */
	if (dir == DMA_TO_DEVICE) {
		engine_id_expected = MDLX_ID_H2C;
		engine = &mdev->engine_h2c[channel];
	} else {
		offset += H2C_CHANNEL_OFFSET;
		engine_id_expected = MDLX_ID_C2H;
		engine = &mdev->engine_c2h[channel];
	}

	regs = mdev->bar[mdev->config_bar_idx] + offset;
	...
	rv = engine_init(engine, mdev, offset, dir, channel);
	if (rv != 0) {
...
static int probe_engines(struct mdlx_dev *mdev)
{
	int i;
	int rv = 0;
	...
	/* iterate over channels */
	for (i = 0; i < mdev->h2c_channel_max; i++) {
		rv = probe_for_engine(mdev, DMA_TO_DEVICE, i);
		if (rv)
			break;
	}
	mdev->h2c_channel_max = i;

	for (i = 0; i < mdev->c2h_channel_max; i++) {
		rv = probe_for_engine(mdev, DMA_FROM_DEVICE, i);
		if (rv)
			break;
	}
	mdev->c2h_channel_max = i;
...
void *mdlx_device_open(const char *mname, struct pci_dev *pdev, int *user_max,
		       int *h2c_channel_max, int *c2h_channel_max)
{
	struct mdlx_dev *mdev = NULL;
	int rv = 0;

	pr_info("%s device %s, 0x%p.\n", mname, dev_name(&pdev->dev), pdev);

	/* allocate zeroed device book keeping structure */
	mdev = alloc_dev_instance(pdev);
	if (!mdev)
		return NULL;
	mdev->mod_name = mname;
	mdev->user_max = *user_max;
	mdev->h2c_channel_max = *h2c_channel_max;
	mdev->c2h_channel_max = *c2h_channel_max;

	mdlx_device_flag_set(mdev, MDEV_FLAG_OFFLINE);
	mdev_list_add(mdev);

	if (mdev->user_max == 0 || mdev->user_max > MAX_USER_IRQ)
		mdev->user_max = MAX_USER_IRQ;
	if (mdev->h2c_channel_max == 0 ||
	    mdev->h2c_channel_max > MDLX_CHANNEL_NUM_MAX)
		mdev->h2c_channel_max = MDLX_CHANNEL_NUM_MAX;
	if (mdev->c2h_channel_max == 0 ||
	    mdev->c2h_channel_max > MDLX_CHANNEL_NUM_MAX)
		mdev->c2h_channel_max = MDLX_CHANNEL_NUM_MAX;
	...
	rv = request_regions(mdev, pdev);
	if (rv)
		goto err_regions;

	rv = map_bars(mdev, pdev);
	if (rv)
		goto err_map;
	...
	check_nonzero_interrupt_status(mdev);
	/* explicitely zero all interrupt enable masks */
	channel_interrupts_disable(mdev, ~0);
	user_interrupts_disable(mdev, ~0);
	read_interrupts(mdev);

	rv = probe_engines(mdev);
	if (rv)
		goto err_engines;

	rv = enable_msi_msix(mdev, pdev);
	if (rv < 0)
		goto err_enable_msix;

	rv = irq_setup(mdev, pdev);
	if (rv < 0)
		goto err_interrupts;

	if (!poll_mode)
		channel_interrupts_enable(mdev, ~0);

	/* Flush writes */
	read_interrupts(mdev);

	*user_max = mdev->user_max;
	*h2c_channel_max = mdev->h2c_channel_max;
	*c2h_channel_max = mdev->c2h_channel_max;

	mdlx_device_flag_clear(mdev, MDEV_FLAG_OFFLINE);
	return (void *)mdev;

err_interrupts:
	irq_teardown(mdev);
err_enable_msix:
	disable_msi_msix(mdev, pdev);
err_engines:
	remove_engines(mdev);
err_mask:
	unmap_bars(mdev, pdev);
err_map:
	if (mdev->got_regions)
		pci_release_regions(pdev);
err_regions:
	if (!mdev->regions_in_use)
		pci_disable_device(pdev);
err_enable:
	mdev_list_remove(mdev);
	kfree(mdev);
	return NULL;
}
EXPORT_SYMBOL_GPL(mdlx_device_open);

void mdlx_device_close(struct pci_dev *pdev, void *dev_hndl)
{
	struct mdlx_dev *mdev = (struct mdlx_dev *)dev_hndl;

	dbg_init("pdev 0x%p, mdev 0x%p.\n", pdev, dev_hndl);
	...
	if (mdev->pdev != pdev) {
		dbg_sg("pci_dev(0x%lx) != pdev(0x%lx)\n",
		       (unsigned long)mdev->pdev, (unsigned long)pdev);
	}
...
EXPORT_SYMBOL_GPL(mdlx_device_close);

void mdlx_device_offline(struct pci_dev *pdev, void *dev_hndl)
{
	struct mdlx_dev *mdev = (struct mdlx_dev *)dev_hndl;
	struct mdlx_engine *engine;
	int i;
	...
	pr_info("mdev 0x%p, done.\n", mdev);
}
EXPORT_SYMBOL_GPL(mdlx_device_offline);

void mdlx_device_online(struct pci_dev *pdev, void *dev_hndl)
{
	struct mdlx_dev *mdev = (struct mdlx_dev *)dev_hndl;
	struct mdlx_engine *engine;
	unsigned long flags;
	...
	if (!poll_mode) {
		...
		channel_interrupts_enable(mdev, ~0);
		user_interrupts_enable(mdev, mdev->mask_irq_user);
		read_interrupts(mdev);
	}
...
EXPORT_SYMBOL_GPL(mdlx_device_online);

int mdlx_device_restart(struct pci_dev *pdev, void *dev_hndl)
{
	struct mdlx_dev *mdev = (struct mdlx_dev *)dev_hndl;
...
EXPORT_SYMBOL_GPL(mdlx_device_restart);

int mdlx_user_isr_register(void *dev_hndl, unsigned int mask,
			   irq_handler_t handler, void *dev)
{
	...
	for (i = 0; i < mdev->user_max && mask; i++) {
		...
		mask &= ~bit;
		mdev->user_irq[i].handler = handler;
		mdev->user_irq[i].dev = dev;
	}

	return 0;
}
EXPORT_SYMBOL_GPL(mdlx_user_isr_register);

int mdlx_user_isr_enable(void *dev_hndl, unsigned int mask)
{
	struct mdlx_dev *mdev = (struct mdlx_dev *)dev_hndl;
	...
	mdev->mask_irq_user |= mask;
	/* enable user interrupts */
	user_interrupts_enable(mdev, mask);
	read_interrupts(mdev);

	return 0;
}
EXPORT_SYMBOL_GPL(mdlx_user_isr_enable);

int mdlx_user_isr_disable(void *dev_hndl, unsigned int mask)
{
	struct mdlx_dev *mdev = (struct mdlx_dev *)dev_hndl;
	...
	mdev->mask_irq_user &= ~mask;
	user_interrupts_disable(mdev, mask);
	read_interrupts(mdev);

	return 0;
}
EXPORT_SYMBOL_GPL(mdlx_user_isr_disable);

#ifdef __LIBMDLX_MOD__
static int __init mdlx_base_init(void)
{
	pr_info("%s", version);
	return 0;
}

static void __exit mdlx_base_exit(void)
{
	pr_info("%s", __func__);
}

module_init(mdlx_base_init);
module_exit(mdlx_base_exit);
#endif
/* makes an existing transfer cyclic */
static void mdlx_transfer_cyclic(struct mdlx_transfer *transfer)
{
...
static int transfer_monitor_cyclic(struct mdlx_engine *engine,
				   struct mdlx_transfer *transfer,
				   int timeout_ms)
{
	struct mdlx_result *result;
...
static int copy_cyclic_to_user(struct mdlx_engine *engine, int pkt_length,
...
static int complete_cyclic(struct mdlx_engine *engine, char __user *buf,
			   size_t count)
{
	struct mdlx_result *result;
	int pkt_length = 0;
...
ssize_t mdlx_engine_read_cyclic(struct mdlx_engine *engine, char __user *buf,
...
int mdlx_cyclic_transfer_setup(struct mdlx_engine *engine)
{
	...
	engine->cyclic_req = mdlx_init_request(&engine->cyclic_sgt, 0);
	if (!engine->cyclic_req) {
...
#ifdef __LIBMDLX_DEBUG__
	mdlx_request_cb_dump(engine->cyclic_req);
#endif
	...
	memset(engine->cyclic_result, 0,
	       CYCLIC_RX_PAGES_MAX * sizeof(struct mdlx_result));
	...
	for (i = 0; i < xfer->desc_num; i++) {
		rc = mdlx_desc_control_clear(xfer->desc_virt + i, LS_BYTE_MASK);
		if (rc < 0) {
			pr_err("Failed to clear desc control\n");
			goto err_out;
		}
		rc = mdlx_desc_control_set(xfer->desc_virt + i,
					   MDLX_DESC_EOP | MDLX_DESC_COMPLETED);
		if (rc < 0) {
			pr_err("Failed to set desc control\n");
			goto err_out;
		}
...
err_out:
	if (engine->cyclic_req) {
		mdlx_request_free(engine->cyclic_req);
...
static int cyclic_shutdown_polled(struct mdlx_engine *engine)
{
	int rv;
	...
	do {
		...
		schedule();
	} while (engine->status & MDLX_STAT_BUSY);
...
static int cyclic_shutdown_interrupt(struct mdlx_engine *engine)
{
	int rc;
...
int engine_addrmode_set(struct mdlx_engine *engine, unsigned long arg)
{
	int rv;
	unsigned long dst;
	u32 w = MDLX_CTRL_NON_INCR_ADDR;

	dbg_perf("IOCTL_MDLX_ADDRMODE_SET\n");
